...
- **Endless Garden:** wired the real build photos (full CAD, assembled prototype) in the gallery,
  plus a dedicated **UI carousel** section with all seven LCD screens (intro, harvest, pH, water,
  nutrient, refill, settings); rewrote My Contributions to Victor's exact role (IR harvest sensor,
  grow lights, LCD UI + Arduino, code integration, CAD contribution and build; plug dispenser as
  support to a teammate), including the display pivot story; trimmed the note to focus on Victor's
  role. Desk report PDF reduced to ~2.7 MB.
